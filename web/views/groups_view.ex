defmodule Thegm.GroupsView do
  ...
  def full_json(group) do
    %{type: "group", attributes: %{name: group.name, address: %{street1: group.street1, street2: group.street2, city: group.city, state: group.state, country: group.country, zip: group.zip}, geo: %{lat: group.lat, lon: group.lon}, contact: %{email: group.email, phone: group.phone}, games: group.games}, relationships: relationships(group.id)}
  end
